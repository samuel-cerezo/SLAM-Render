...
from utils.transformations_utils import load_yaml_transformations, load_poses, pose_to_homogeneous, transform_poses_left, transform_poses_right
...
class OPENLORISParser:
    ...
    def load_poses(self, datapath, frame_rate=-1):
        ...
        depth_list = os.path.join(datapath, "aligned_depth.txt")
...
class iisyFusionParser:
    ...
    def load_poses(self, datapath, frame_rate=-1):
        ...
        #image_list = os.path.join(datapath, "color.txt")
        #depth_list = os.path.join(datapath, "aligned_depth.txt")
        association_list = os.path.join(datapath, "associations.txt")
        # ------- transformations ------
        ...
        #T_flange_markers_rgb = np.linalg.inv(T_rgb_flange_markers)
        
        
        association_data = np.loadtxt(association_list, delimiter=" ", dtype=np.unicode_, skiprows=1)

        image_data = association_data[:,:2]
        depth_data = association_data[:,2:]
        pose_data = self.parse_list(pose_list, skiprows=1)
        pose_vecs = pose_data[:, 0:].astype(np.float64)
        tstamp_image = image_data[:, 0].astype(np.float64)
        tstamp_depth = depth_data[:, 0].astype(np.float64)
        ...
        #print(f"--- debug: {len(indicies)} --- ")

        for ix in indicies:
            (i, j, k) = associations[ix]
            self.color_paths += [os.path.join(datapath, image_data[i, 1])]
            self.depth_paths += [os.path.join(datapath, depth_data[j, 1])]
            self.timestamps += [tstamp_image[associations[ix][0]]]
            #print(f"--- debug: {self.timestamps} ---\n")

            #quat = pose_vecs[k][4:]
            quat = pose_vecs[k][1:5]
            #trans = pose_vecs[k][1:4]
            #trans = pose_vecs[k][5:] / 1000
            trans = pose_vecs[k][5:]

            T_hom = pose_to_homogeneous(quat, trans)
            T = T_hom @ np.linalg.inv(T_rgb_flange_markers)
            T[:3, 3] = T[:3, 3] / 1000  #mm to m

            #T = trimesh.transformations.quaternion_matrix(np.roll(quat, 1))
            #T[:3, 3] = trans 
            #T = T @ T_flange_markers_rgb
...
class OPENLORISDataset(MonocularDataset):
    def __init__(self, args, path, config):
        super().__init__(args, path, config)
        dataset_path = config["Dataset"]["dataset_path"]
        parser = OPENLORISParser(dataset_path)
        self.num_imgs = parser.n_img
        self.color_paths = parser.color_paths
        self.depth_paths = parser.depth_paths
        self.poses = parser.poses

class iisyFusionDataset(MonocularDataset):
    def __init__(self, args, path, config):
        super().__init__(args, path, config)
        dataset_path = config["Dataset"]["dataset_path"]
        parser = iisyFusionParser(dataset_path)
        self.num_imgs = parser.n_img
        self.color_paths = parser.color_paths
        self.depth_paths = parser.depth_paths
        self.poses = parser.poses
...
def load_dataset(args, path, config):
    ...
    elif config["Dataset"]["type"] == "openloris":
        return OPENLORISDataset(args, path, config)
    elif config["Dataset"]["type"] == "iisyFusion":
        return iisyFusionDataset(args, path, config)
